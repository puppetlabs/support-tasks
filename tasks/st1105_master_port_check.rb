...
portdesc = {
    "8140" => "The master uses this port to accept inbound traffic/requests from agents. The console sends requests to the master on this port. Certificate requests are passed over this port unless ca_port is set differently.\nPuppet Server status checks are sent over this port.",
...
ports.split(/\s*,\s*/).each { |port_no|
    # If port is open
    if port_test(destination, port_no)
        result = {
